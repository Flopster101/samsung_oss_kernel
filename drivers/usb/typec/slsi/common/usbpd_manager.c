...
void usbpd_manager_set_source_cap(int size_of_power_list, POWER_LIST power_list[])
{
	...
	for(i = 0; i<size_of_power_list; i++) {
		...
		case VPDO_TYPE:
			...
			break;
		default:
			usbpd_info("%s, invalid pdo_type(%d)\n", __func__, power_list[i].pdo_type);
			init_source_cap_data(manager);
			return;
		}
	}
...
void usbpd_manager_select_pdo(int num)
{
	...
	if (vbus_short) {
		usbpd_info("%s : PDO(%d) is ignored because of vbus short\n",
	...
	usbpd_info(" %s : PDO(%d) is selected to change\n", __func__, pd_data->pd_noti.sink_status.selected_pdo_num);
...
int usbpd_manager_select_pps(int num, int ppsVol, int ppsCur)
{
	...
	if (num > pd_data->pd_noti.sink_status.available_pdo_num) {
		usbpd_info("%s: request pdo num(%d) is higher that available pdo.\n", __func__, num);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (ppsVol > pd_data->pd_noti.sink_status.power_list[num].max_voltage) {
		usbpd_info("%s: ppsVol is over(%d, max:%d)\n",
		...
		usbpd_info("%s: ppsVol is under(%d, min:%d)\n",
	...
	if (ppsCur > pd_data->pd_noti.sink_status.power_list[num].max_current) {
		usbpd_info("%s: ppsCur is over(%d, max:%d)\n",
		...
		usbpd_info("%s: ppsCur is under(%d, 0)\n",
			__func__, ppsCur);
		ppsCur = 0;
	}
	...
	usbpd_info(" %s : PPS PDO(%d), voltage(%d), current(%d) is selected to change\n",
		__func__, pd_data->pd_noti.sink_status.selected_pdo_num, ppsVol, ppsCur);

	if ((ppsVol == 5000) && (ppsCur == 3000) && (pd_data->pps_pd != 2)) {
...
EXPORT_SYMBOL(usbpd_manager_select_pps);
	...
	unsigned int *taMaxVol, unsigned int *taMaxCur, unsigned int *taMaxPwr)
{
	...
	if (!pd_data->pd_noti.sink_status.has_apdo) {
		usbpd_info("%s: pd don't have apdo\n", __func__);
		return -1;
	}
	...
	if (*pdo_pos == 0) {
		...
		if (*pdo_pos == 0) {
			usbpd_info("mv (%d) and ma (%d) out of range of APDO\n",
				*taMaxVol, *taMaxCur);
			ret = -EINVAL;
		}
	} else {
	...
	usbpd_info("%s : *pdo_pos(%d), *taMaxVol(%d), *maxCur(%d), *maxPwr(%d)\n",
...
int usbpd_manager_pps_enable(int num, int ppsVol, int ppsCur, int enable)
{
	...
	if (pd_data->phy_ops.pps_enable == NULL) {
		usbpd_info(" %s : pps_enable function is not present\n", __func__);
		return -ENOMEM;
	}
	...
	if ((pps_enable == enable) && pps_enable) {
		usbpd_info(" %s : auto pps is already enabled\n", __func__);
		goto exit;
	}
...
{
	struct usbpd_data *pd_data = g_pd_data;
	int ret = (PDIC_OPS_FUNC(get_pps_voltage, pd_data) * USBPD_PPS_RQ_VOLT_UNIT);
...
void pdo_ctrl_by_flash(bool mode)
{
	...
	pd_data = g_pd_data;
	if (!pd_data) {
		usbpd_info("%s, pd_data is NULL\n", __func__);
		return;
	}

	manager = &pd_data->manager;
	if (!manager) {
		usbpd_info("%s, manager is NULL\n", __func__);
		return;
	}
...
{
	usbpd_info("%s: call send pr swap msg\n", __func__);
...
void usbpd_manager_match_sink_cap(struct usbpd_data *pd_data)
{
	...
	usbpd_info("%s, src_cur(%dmA) SinkCap volt(%dmV), opcur(%dmA)\n", __func__,
	...
	if ((src_cur <= op_cur) && (op_cur * 10 <= 1000)) {
		...
		usbpd_info("%s, SourceCap cur (%dmA)\n", __func__,
...
/*	PDIC_OPS_PARAM_FUNC(get_power_role, pd_data, &val);		*/
...
int samsung_uvdm_ready(void)
{
	...
	usbpd_info("uvdm ready = %d", uvdm_ready);
#endif
	return uvdm_ready;
}
...
int usbpd_manager_send_samsung_uvdm_message(void *data, const char *buf, size_t size)
{
	...
	if ((buf == NULL)||(size < sizeof(unsigned int))) {
		usbpd_err("%s given data is not valid !\n", __func__);
		return -EINVAL;
	}
	...
	if (data_role == USBPD_UFP) {
		usbpd_err("%s, skip, now data role is ufp\n", __func__);
		return 0;
	}

	data_role = PDIC_OPS_PARAM_FUNC(get_power_role, pd_data, &power_role);
	...
	if (manager->uvdm_data_obj[1].sec_uvdm_header.data_type == SEC_UVDM_SHORT_DATA) {
		usbpd_info("%s - process short data!\n", __func__);
		// process short data
		...
		usbpd_info("%s - process long data!\n", __func__);
		// process long data
		// phase 1. fill message header
...
int samsung_uvdm_write(void *data, int size)
{
	...
	if (size <= 1) {
		...
		while (cur_set_num <= need_set_cnt) {
			...
			usbpd_info("%s current set data size: %d, total data size %d, current uvdm set num %d\n",
			...
			if (SEC_UVDM_RES_HEADER.data_type == TYPE_LONG) {
				if (SEC_UVDM_RES_HEADER.cmd_type == RES_ACK) {
					...
					if (SEC_UVDM_RX_HEADER.result_value != RX_ACK)
						usbpd_err("%s Busy or Nak received.\n", __func__);
				} else
					usbpd_err("%s Response type is wrong.\n", __func__);
			} else {
				if ( SEC_UVDM_RES_HEADER.cmd_type == RES_ACK)
					usbpd_err("%s Short packet: ack received\n", __func__);
				else
					usbpd_err("%s Short packet: Response type is wrong\n", __func__);
			}
		/* Dir: out */
		} else {
...
void usbpd_manager_plug_attach(struct device *dev, muic_attached_dev_t new_dev)
{
	...
	manager->first_noti_sent = true;
#else
	struct usbpd_data *pd_data = dev_get_drvdata(dev);
	struct usbpd_manager_data *manager = &pd_data->manager;

	usbpd_info("%s: usbpd plug attached\n", __func__);
...
void usbpd_manager_plug_detach(struct device *dev, bool notify)
{
	...
	usbpd_info("%s: usbpd plug detached\n", __func__);
...
void usbpd_manager_acc_detach(struct device *dev)
{
	...
	if ( manager->acc_type != PDIC_DOCK_DETACHED ) {
		usbpd_info("%s: schedule_delayed_work\n", __func__);
		...
		usbpd_manager_event_type event)
{
	...
	case MANAGER_CAP_MISMATCH:
		...
		break;
	default:
		usbpd_info("%s: not matched event(%d)\n", __func__, event);
	}
}
...
bool usbpd_manager_pps_request(struct usbpd_data *pd_data)
{
	struct usbpd_manager_data *manager = &pd_data->manager;

	pd_data->pps_pd = 2;
	schedule_delayed_work(&manager->pps_request_handler, msecs_to_jiffies(5000));

	return true;
}
EXPORT_SYMBOL(usbpd_manager_pps_request);
...
void usbpd_manager_turn_off_power_sink(struct usbpd_data *pd_data)
{
	...
	usbpd_info("%s: usbpd sink turn off\n", __func__);
...
void usbpd_manager_acc_handler_cancel(struct device *dev)
{
	...
	if (manager->acc_type != PDIC_DOCK_DETACHED) {
		usbpd_info("%s: cancel_delayed_work_sync\n", __func__);
	...
	if (manager->acc_type == PDIC_DOCK_DETACHED) {
		if (vid == SAMSUNG_VENDOR_ID) {
			...
			case GEARVR_PRODUCT_ID_5:
				...
				usbpd_info("%s : Samsung Gear VR connected.\n", __func__);
#if defined(CONFIG_USB_HW_PARAM)
				if (o_notify)
			...
			case DEXDOCK_PRODUCT_ID:
				...
				usbpd_info("%s : Samsung DEX connected.\n", __func__);
#if defined(CONFIG_USB_HW_PARAM)
				if (o_notify)
			...
			case HDMI_PRODUCT_ID:
				...
				usbpd_info("%s : Samsung HDMI connected.\n", __func__);
				break;
			case MPA2_PRODUCT_ID:
				acc_type = PDIC_DOCK_NEW;
				usbpd_info("%s : Samsung MPA2 connected. need to SRCCAP_CHANGE\n", __func__);
			...
			default:
				acc_type = PDIC_DOCK_NEW;
				usbpd_info("%s : default device connected.\n", __func__);
				break;
			}
			...
			case MPA_PRODUCT_ID:
				...
				usbpd_info("%s : Samsung MPA connected.\n", __func__);
				break;
			default:
				acc_type = PDIC_DOCK_NEW;
				usbpd_info("%s : default device connected.\n", __func__);
			...
			acc_type = PDIC_DOCK_NEW;
			usbpd_info("%s : unknown device connected.\n",	__func__);
		}
...
int usbpd_manager_get_identity(struct usbpd_data *pd_data)
{
	...
	usbpd_info("khos %s, Vendor_ID : 0x%x, Product_ID : 0x%x, Device Version : 0x%x\n",
			__func__, manager->Vendor_ID, manager->Product_ID, manager->Device_Version);
	if (usbpd_manager_check_accessory(manager))
		usbpd_info("%s, Samsung Accessory Connected.\n", __func__);
...
int usbpd_manager_enter_mode(struct usbpd_data *pd_data)
{
	struct policy_data *policy = &pd_data->policy;
	struct usbpd_manager_data *manager = &pd_data->manager;
	uint16_t svid = manager->Standard_Vendor_ID = policy->rx_data_obj[0].structured_vdm.svid;
	int ret;
	...
	case SAMSUNG_VENDOR_ID:
		pdic_event_work(pd_data, PDIC_NOTIFY_DEV_ALL,
				PDIC_NOTIFY_ID_SVID_INFO, svid/*Standard Vendor ID*/, 0, 0);
		ret = 0;
		break;
...
data_obj_type usbpd_manager_select_capability(struct usbpd_data *pd_data)
{
	...
	usbpd_info("%s, selected(%d), current(%d)\n", __func__,
			pd_data->pd_noti.sink_status.selected_pdo_num, pd_data->pd_noti.sink_status.current_pdo_num);
#if IS_ENABLED(CONFIG_PDIC_PD30)
...
int usbpd_manager_evaluate_capability(struct usbpd_data *pd_data)
{
		...
		if (power_type == POWER_TYPE_FIXED
			...
			if (power_type == POWER_TYPE_FIXED) {
				...
				if (!manager->support_vpdo || max_volt * max_volt_unit[power_type] > AVAILABLE_VOLTAGE)
					...
					pdic_sink_status->power_list[i + 1].accept = true;
			}  else if (power_type == POWER_TYPE_PPS) {
				pdic_sink_status->power_list[i + 1].accept = true;
			}
	...
	if ((available_pdo_num == 1) &&
		...
		usbpd_info("%s, TX NAK to Rx svid_req\n", __func__);
		policy->not_support_svid_ack = 1;
	}
	...
	/* for First Negotiation, PR_Swap, HardReset -> select 5V PDO */
	if (pdic_sink_status->selected_pdo_num == 0)
		pdic_sink_status->selected_pdo_num = 1;

	/* if SrcCap Changed when Rx SrcCap msg,
	 *  goto Default 5V & EVENT_PD_SINK_CAP
	 *  (duing PR_Swap, HardReset, just Rx)
	 */
	if (manager->first_noti_sent) {
		if (src_cap_changed || ((available_pdo_num > 0) &&
			...
			if(manager->vpdo_received == 1) {
				for(i=0; i < available_pdo_num; i++) {
					if(pdic_sink_status->power_list[i + 1].pdo_type == VPDO_TYPE) {
						pdic_sink_status->selected_pdo_num = i + 1;
						break;
					}
				}
			}
...
int usbpd_manager_match_request(struct usbpd_data *pd_data)
{
	...
	if (pos != 1) {
		usbpd_info("Pos Number is not 1, Pos : %d\n", pos);
		return -1;
	}
	supply_type = pd_data->source_data_obj[pos - 1].power_data_obj_supply_type.supply_type;
	...
	if (supply_type == POWER_TYPE_FIXED) {
		...
		pd_data->policy.requested_pdo_num = pd_data->source_request_obj.request_data_object.object_position;
		usbpd_info("REQUEST: VARIABLE\n");
	}
	else if (supply_type == POWER_TYPE_PPS) {
		pd_data->policy.requested_pdo_type = APDO_TYPE;
		pd_data->policy.requested_pdo_num = pd_data->source_request_obj.request_data_object.object_position;
		usbpd_info("REQUEST: PPS\n");
	}
	else if (supply_type == POWER_TYPE_BATTERY) {
		usbpd_info("REQUEST: BATTERY\n");
		goto log_battery;
	} else {
		usbpd_info("REQUEST: UNKNOWN Supply type.\n");
		return -1;
	}

    /* Tx Source PDO */
	src_max_current = pd_data->source_data_obj[0].power_data_obj.max_current;
...
void usbpd_manager_response_req_pdo(struct usbpd_data *pd_data, int req_pdo_type, int req_pdo_num)
{
	...
	usbpd_info("D2D: %s: snk pdo(sel_pdo:%d, pdo_type:%d)\n",
...
void usbpd_manager_forced_src_cap(int max_cur)
{
	POWER_LIST power_list[2];
	int size_of_power_list;
	...
	power_list[0].pdo_type = FPDO_TYPE;
	power_list[0].max_voltage = 5000;
	power_list[0].max_current = max_cur;
	power_list[0].comm_capable = 1;
	power_list[0].suspend = 1;
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG) && IS_ENABLED(CONFIG_USB_TYPEC_MANAGER_NOTIFIER)
	usbpd_manager_set_source_cap(size_of_power_list, power_list);
...
void usbpd_manager_vpdo_auth(int auth, int d2d_type)
{
	...
	usbpd_info("@D2D: %s: [DEBUG] power_role(%d), manager->auth_type(%d), auth(%d)\n",
		__func__, power_role, manager->auth_type, auth);

	if (power_role == PDIC_SOURCE) {
		if (((manager->auth_type == AUTH_HIGH_PWR) && (auth == AUTH_LOW_PWR)) ||
			...
			usbpd_info("@D2D: %s: change src %s -> %s\n", __func__,
				(auth == AUTH_LOW_PWR) ? "HIGH PWR" : "LOW PWR",
				(auth == AUTH_LOW_PWR) ? "LOW PWR" : "HIGH PWR");
		}
	} else if ((power_role == PDIC_SINK) &&
		(auth == AUTH_HIGH_PWR)) {
		usbpd_info("@D2D: %s: preset vpdo auth for prswap snk to src\n", __func__);
	}
	...
	if (power_role != PDIC_SINK) {
		if ((manager->auth_type == AUTH_HIGH_PWR) && (auth == AUTH_NONE)) {
			usbpd_manager_send_new_src_cap(auth, d2d_type);
			usbpd_info("@D2D: %s: set to default src cap\n", __func__);
		}
	}

	usbpd_info("@D2D: %s: vpdo auth set (%d, %d)\n", __func__, auth, d2d_type);
...
void usbpd_manager_short_check(struct usbpd_data *pd_data)
{
	...
	usbpd_info("%s, call short check\n", __func__);
	schedule_delayed_work(&manager->short_check_work, msecs_to_jiffies(0));

	return;
}

static bool usbpd_manager_check_boost_enable(bool enable_vbus, int auth_t, int req_pdo, int d2d_t)
{
	if (!enable_vbus)
		return false;
	...
	usbpd_info("@D2D: %s: enable_vbus(%d), auth_t(%d), req_pdo(%d), d2d_t(%d)\n",
...
static bool usbpd_manager_check_boost_off(bool enable_vbus, int auth_t, int req_pdo, int d2d_t)
{
	if (!enable_vbus)
		return true;
...
static bool usbpd_manager_src_otg_type(bool enable, int auth_t, int req_pdo, int d2d_t)
{
	if ((auth_t == AUTH_NONE) || (d2d_t != D2D_SRCSNK))
		return enable;

	usbpd_info("@D2D: %s: enable(%d), auth_t(%d), req_pdo(%d), d2d_t(%d)\n",
...
void usbpd_manager_vbus_turn_on_ctrl(void *_data, bool enable)
{
	...
	pd_data = dev_get_drvdata(pd_data->dev);
	if (!pd_data) {
		usbpd_err("@D2D: %s: pd_data is null!!\n", __func__);
		return;
	}

	manager = &pd_data->manager;
	if (!manager) {
		usbpd_err("@D2D: %s: manager_data is null!!\n", __func__);
		return;
	}
	...
	usbpd_info("%s %d, enable=%d\n", __func__, __LINE__, enable);

#if IS_ENABLED(CONFIG_USB_HOST_NOTIFY)
	if (o_notify && o_notify->booting_delay_sec && enable) {
		usbpd_info("%s %d, is booting_delay_sec. skip to control booster\n",
			__func__, __LINE__);
		reserve_booster = 1;
	...
	if (psy_otg) {
		...
		PDIC_OPS_PARAM_FUNC(set_is_otg_vboost, pd_data, enable);
	...
	if (ret) {
		usbpd_err("%s: fail to set power_suppy ONLINE property(%d)\n",
			__func__, ret);
	} else {
...
EXPORT_SYMBOL_GPL(usbpd_manager_vbus_turn_on_ctrl);
...
static int of_usbpd_manager_dt(struct usbpd_manager_data *_data)
{
	...
	if (np == NULL) {
		...
		if (ret < 0) {
			...
			usbpd_err("%s error reading sink_cap_max_volt %d\n",
...
#if IS_ENABLED(CONFIG_S2MU106_TYPEC_WATER)
			(*val).intval = PDIC_OPS_FUNC(ops_water_check, pd_data);
#endif
			break;
		case POWER_SUPPLY_LSI_PROP_DRY_CHECK:
#if IS_ENABLED(CONFIG_S2MU106_TYPEC_WATER)
			(*val).intval = PDIC_OPS_FUNC(ops_dry_check, pd_data);
#endif
		default:
			break;
		}
...
#if IS_ENABLED(CONFIG_S2MU106_TYPEC_WATER)
			PDIC_OPS_PARAM_FUNC(water_opmode, pd_data, val->intval);
#endif
			break;
...
int usbpd_manager_psy_init(struct usbpd_data *_data, struct device *parent)
{
	struct power_supply_config psy_cfg = {};
	int ret = 0;

	if (_data == NULL || parent == NULL) {
		usbpd_err("%s NULL data\n", __func__);
		return -1;
	}

	_data->pdic_desc.name           = "usbpd-manager";
	_data->pdic_desc.type           = POWER_SUPPLY_TYPE_UNKNOWN;
	_data->pdic_desc.get_property   = usbpd_manager_get_property;
	_data->pdic_desc.set_property   = usbpd_manager_set_property;
	...
	if (IS_ERR(_data->psy_pdic)) {
		...
		usbpd_err("%s: Failed to Register psy_pdic, ret : %d\n",
			__func__, ret);
	}
	return ret;
}
EXPORT_SYMBOL_GPL(usbpd_manager_psy_init);
...
int usbpd_init_manager(struct usbpd_data *pd_data)
{
	...
	usbpd_info("%s\n", __func__);
	if (manager == NULL) {
		usbpd_err("%s, usbpd manager data is error!!\n", __func__);
		return -ENOMEM;
	} else
	...
	pd_data->pd_noti.sink_status.fp_sec_pd_change_src = usbpd_manager_forced_src_cap;
#endif
#endif
